...
describe('send slice', () => {
  ...
  describe('simple actions', () => {
    ...
    it('updateSend should completely override state', () => {
      ...
      store.dispatch(updateSend(overriddenState));
      const state = store.getState();
      expect(state.send).toMatchObject(overriddenState);
    });
  ...
  describe('selectors', () => {
    ...
    it('getSendFrom should return "from" from state', async () => {
      ...
      store.dispatch(updateSend(overriddenState));
      expect(getSendFrom(store.getState())).toBe('0x00');
    });
